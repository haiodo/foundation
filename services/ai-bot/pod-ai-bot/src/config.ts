...
const config: Config = (() => {
  const params: Partial<Config> = {
    ...
    OpenAITranslateModel: (process.env.OPENAI_TRANSLATE_MODEL ?? process.env.OPENAI_MODEL ?? 'gpt-4o-mini') as OpenAI.ChatModel,
    OpenAISummaryModel: (process.env.OPENAI_SUMMARY_MODEL ?? process.env.OPENAI_MODEL ?? 'gpt-4o-mini') as OpenAI.ChatModel,
