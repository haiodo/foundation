...
export class WorkspaceClient {
  ...
  private async getHistory (personUuid: PersonUuid): Promise<PersonHistoryRecord> {
    if (this.historyMap.has(personUuid)) {
      return this.historyMap.get(personUuid) ?? {
        assistantMemory: '',
        userMemory: '',
        sharedContext: '',
        history: []
    ...
    try {
      const personHistory: PersonHistoryRecord = JSON.parse(Buffer.concat(await this.storage.read(this.ctx, this.wsIds, 'ai-bot-phr-' + personUuid)).toString())
  ...
  async saveHistory (personUuid: PersonUuid, history: PersonHistoryRecord): Promise<void> {
    await this.storage.put(this.ctx, this.wsIds, 'ai-bot-phr-' + personUuid, JSON.stringify(history), 'application/json')
  }
  ...
  async processMessageEvent (event: AIEventRequest, control?: ConsumerControl): Promise<void> {
    ...
    const { user, objectId, objectClass, messageClass, messageId } = event
    ...
    if (contextMode !== 'direct') {
      ...
      const msg = await this.client?.findOne<ThreadMessage>(objectClass, { _id: objectId as Ref<ThreadMessage> })
      if (msg !== undefined) {
        if (messageClass === chunter.class.ThreadMessage || messageClass === chunter.class.ChatMessage) {
          useHistory.push({
            role: 'user',
      ...
      // TODO: Add support for new chat.
      const lastMessages = (await this.client?.findAll(chunter.class.ChatMessage, { attachedTo: objectId, attachedToClass: objectClass }, { limit: 500, sort: { modifiedOn: SortingOrder.Descending } })) ?? []

      lastMessages.sort((a, b) => a.modifiedOn - b.modifiedOn)

      const personIds = new Set(lastMessages.map(it => it.modifiedBy))

      const socialIds = toIdMap((await this.client?.findAll(contact.class.SocialIdentity, { _id: { $in: Array.from(personIds) as any } })) ?? [])

      const employeesInChannel = (await this.client?.findAll(contact.class.Person, { _id: { $in: Array.from(socialIds.values()).map(it => it.attachedTo) } })) ?? []
